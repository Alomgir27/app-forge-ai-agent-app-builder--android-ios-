...
const generateCode = (data, appConfig, target) => {
    const files = [];
    // 1. Generate Source Code
    const generateNode = (node) => {
        const component = registry_1.ComponentRegistry[node.type];
        if (!component)
            return '';
        ...
        if (node.props.content && Array.isArray(node.props.content)) {
            childrenCode = node.props.content.map((child) => generateNode(child)).join('\n');
        }
...
const styles = StyleSheet.create({
  container: {
    ...
    justifyContent: 'center',
        ...
        // package.json
        const dependencies = resolver_1.VersionResolver.resolve('react-native', []);
        files.push({
            path: 'package.json',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        ...
        // pubspec.yaml
        const dependencies = resolver_1.VersionResolver.resolve('flutter', []);
        let depString = '';
